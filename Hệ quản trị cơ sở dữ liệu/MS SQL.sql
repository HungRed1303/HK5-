...
CREATE TABLE PHIEUMUAHANG (
	MaPhieu INT, 
  	...
  	MaNV INT NOT NULL,
  	HanSuDung DATETIME,
  	TrangThai NVARCHAR CHECK (TrangThai IN ('Đã sử dụng', 'Chưa sử dụng', 'Hết hạn')),
	PRIMARY KEY (MaPhieu)
)
...
CREATE TABLE DONDATNSX (
	MaDDH INT, 
  	...
  	MaSP INT NOT NULL,
  	NgayDat DATETIME,
  	TinhTrang NVARCHAR CHECK(TinhTrang IN ('Chưa giao', 'Đã giao')),
  	MaNV INT NOT NULL,
  	PRIMARY KEY (MaDDH)
)
...
CREATE TABLE DONHANG (
	MaDH INT,
  	NgayDat DATETIME,
  	NgayGiao DATETIME,
  	TinhTrang NVARCHAR CHECK (TinhTrang IN ('Đang xử lý', 'Đang vận chuyển', 'Đã giao', 'Đã hủy')),
...
CREATE TABLE KHUYENMAI (
	MaKhuyenMai INT,
  	...
  	TiLe FLOAT NOT NULL,
  	SLToiDa INT NOT NULL,
  	TinhTrang NVARCHAR CHECK (TinhTrang IN ('Đang diễn ra', 'Kết thúc')), 
  	SLDaBan INT NOT NULL,
  	LoaiKM NVARCHAR CHECK (LoaiKM IN ('Flash-sale', 'Combo-sale', 'Member-sale')),
  	MaNV INT NOT NULL,
