...
select* from EMPLOYEE where EmployeeName Like '%Tran%' and Salary > 1 
...
CREATE PROCEDURE AddBranch
    ...
    @CloseHour TIME,
    @PhoneNumber CHAR(15),
    @HasCarParking VARCHAR(10),
    @HasMotorParking VARCHAR(10),
    @AreaID INT,
    @ManagerID INT,
    @HasDeliveryService VARCHAR(10)
AS
BEGIN
    -- Kiểm tra OpenHour và CloseHour
    IF @OpenHour >= @CloseHour
    BEGIN
        PRINT 'Error: OpenHour must be earlier than CloseHour.';
        RETURN;
    END

    -- Kiểm tra giá trị hợp lệ cho các trường YES/NO
    IF @HasCarParking NOT IN ('YES', 'NO') OR
       @HasMotorParking NOT IN ('YES', 'NO') OR
       @HasDeliveryService NOT IN ('YES', 'NO')
    BEGIN
        PRINT 'Error: HasCarParking, HasMotorParking, and HasDeliveryService must be either YES or NO.';
        RETURN;
    END

    -- Chèn thông tin chi nhánh mới
    INSERT INTO BRANCH (
		BranchID,
        BranchName,
        BranchAddress,
